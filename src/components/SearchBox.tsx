...
export function SearchBox() {
    ...
    const timer = window.setTimeout(() => {
      Promise.all([tmdb.searchCatalog(query, 1), catalog.people(query, 1)])
        .then(([found, persons]) => {
          if (!alive) return;
          setTitles(
            applySearch(found.results, query, {
              kind: "all",
              year: "",
              genre: null,
              minScore: 0,
  ...
  return (
    <form
      ref={root}
      onSubmit={onSubmit}
      className="relative ml-auto min-w-0 flex-1 max-w-sm"
    >
      <input
        value={q}
        onChange={(e) => setQ(e.target.value)}
        onFocus={() => {
          if (titles.length || people.length) setOpen(true);
        }}
        placeholder="Поиск"
      ...
      {open && (titles.length || people.length) ? (
        <div className="absolute right-0 z-50 mt-2 w-full overflow-hidden rounded-2xl border border-hairline bg-card shadow-[0_18px_50px_rgba(0,0,0,0.55)] sm:w-[380px]">
            ...
            return (
              ...
              >
                {item.poster_path ? (
                  ...
                  <div className="h-12 w-8 rounded bg-canvas" />
                )}
          ...
          <button
            type="submit"
            className="block w-full border-t border-hairline px-3 py-2 text-left text-xs text-accent"
          >
            Все результаты
          </button>
        </div>
