...
function sortFor(type: MediaType, sort: SearchFilters['sort']) {
  ...
  if (sort === 'year') return type === 'tv' ? 'first_air_date.desc' : 'primary_release_date.desc'
...
export const catalog = {
  ...
  browseFiltered: async (filters: SearchFilters, page = 1): Promise<TmdbPage<TmdbItem>> => {
    const types: MediaType[] = filters.kind === 'all' ? ['movie', 'tv'] : [filters.kind]
    const pages = await Promise.all(types.map((type) =>
      request<TmdbPage<TmdbItem>>(`/discover/${type}`, { page, ...extras(type, filters) }),
    ))
    const results: TmdbItem[] = []
    pages.forEach((pack, index) => {
      const type = types[index]
      pack.results.forEach((item) => results.push({ ...item, media_type: type }))
    })
    return {
      page,
      results,
