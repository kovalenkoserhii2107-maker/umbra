    ...
    <div className="rise space-y-8">
      <div>
        ...
        <p className="mt-2 text-sm text-mute">
          Без запроса — просто выбери, что смотреть.
        </p>
      </div>
      <section>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {SEARCH_GENRES.map((genre) => (
            <Link
              key={genre.id}
              to={`/guide/${genre.id}`}
              className="rounded-2xl border border-hairline bg-card px-4 py-4 text-sm hover:border-accent/50"
            >
              {genre.label}
            </Link>
          ))}
        ...
        <div className="flex flex-wrap gap-2">
          {DECADES.map((year) => (
            <Link
              key={year}
              to={`/guide/year/${year}`}
              className="rounded-full border border-hairline px-4 py-2 text-sm text-mute hover:text-ink"
            >
              {year}-е
            </Link>
          ))}
...
export function GuideListPage() {
  ...
  const movieItems = (movies.data?.results ?? []).map((item) => ({
    ...item,
    media_type: "movie" as const,
  }));
  const showItems = (shows.data?.results ?? []).map((item) => ({
    ...item,
  ...
  return (
    <div className="rise">
      ...
      <h1 className="mt-1 text-3xl tracking-tight">
        {genre?.label || (decade ? `${decade}-е` : "Подборка")}
      </h1>
