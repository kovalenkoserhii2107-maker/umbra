...
export function LibraryPage() {
  ...
  return (
    <div className="rise">
      <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-accent">полка</p>
      <h1 className="mt-1 text-3xl tracking-tight">Личная медиатека</h1>
      <p className="mt-2 text-sm text-mute">
        {items.length} {plural(items.length, 'тайтл', 'тайтла', 'тайтлов')} · только на этом устройстве
      </p>
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`rounded-full border px-3 py-1 font-mono text-[11px] uppercase tracking-[0.12em] ${
              filter === f.id ? 'border-ink bg-ink text-canvas' : 'border-hairline text-mute'
            }`}
          >
            {f.label}
          </button>
      ...
      {list.length === 0 ? (
        ...
        <div className="space-y-2">
          {list.map((item) => (
            ...
            >
              {item.poster ? (
                <img src={item.poster} alt="" className="h-20 w-14 rounded-lg object-cover" />
              ) : (
              ...
              <div className="min-w-0 flex-1 py-1">
                ...
                {item.note ? <p className="mt-1 line-clamp-1 text-sm text-mute">{item.note}</p> : null}
              </div>
            </Link>
          ))}
        </div>
