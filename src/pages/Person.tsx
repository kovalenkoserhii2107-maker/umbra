...
export function PersonPage() {
  ...
  const works = useMemo(() => {
    ...
    for (const item of person.combined_credits.crew || []) {
      ...
      if (!prev) {
        ...
        map.set(key, { ...prev, role: 'режиссёр', job: 'Director', department: 'Directing' })
      }
    }
  ...
  const filtered = works.filter((w) => {
    if (tab === 'director') return w.job === 'Director' || w.department === 'Directing'
  ...
  return (
    <div className="rise pb-8">
      <div className="flex gap-4">
        {person.profile_path ? (
          <img src={posterUrl(person.profile_path, 'w185')} alt="" className="h-40 w-28 rounded-2xl object-cover" />
        ) : (
          <div className="h-40 w-28 rounded-2xl border border-hairline bg-card" />
        )}
        <div className="min-w-0">
          <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-accent">фильмография</p>
          <h1 className="mt-1 text-3xl tracking-tight">{person.name}</h1>
          <p className="mt-2 text-sm text-mute">
            {[person.known_for_department, person.birthday, person.place_of_birth].filter(Boolean).join(' · ')}
          </p>
        </div>
      </div>
      {person.biography ? <p className="mt-5 max-w-3xl text-sm leading-7 text-mute">{person.biography}</p> : null}
        ...
        {([
          ...
          <button
            ...
            className={`rounded-full border px-3 py-1 font-mono text-[11px] uppercase tracking-[0.12em] ${
              tab === id ? 'border-ink bg-ink text-canvas' : 'border-hairline text-mute'
            }`}
          >
            {label}
          </button>
      ...
      <div className="space-y-2">
        {filtered.map((work) => {
          ...
          return (
            ...
            >
              {work.poster_path ? (
                <img src={posterUrl(work.poster_path, 'w185')} alt="" className="h-20 w-14 rounded-lg object-cover" />
              ) : (
              ...
              <div className="min-w-0 py-1">
                ...
                <p className="font-mono text-[11px] uppercase tracking-wider text-dim">
                  {year || '—'} · {type === 'tv' ? 'сериал' : 'фильм'} · {work.role}
                </p>
              </div>
            </Link>
          )
      ...
      {!filtered.length ? <p className="mt-8 text-sm text-mute">В этой роли записей нет.</p> : null}
    </div>
  )
}
