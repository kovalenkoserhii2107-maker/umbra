...
export function SearchPage() {
  ...
  const queryKey = `${q}:${q ? "" : JSON.stringify(filters)}`;
  ...
  useEffect(() => {
    ...
    setError(null);
    const request = q
      ? Promise.all([tmdb.searchCatalog(q, 1), catalog.people(q, 1)]).then(
          ([data, persons]) => ({
            data,
            persons: persons.results.slice(0, 12),
          }),
        )
      : catalog
          .browseFiltered(filters, 1)
          .then((data) => ({ data, persons: [] as PersonHit[] }));
    request
      .then(({ data, persons }) => {
        if (!alive) return;
  ...
  const shown = useMemo(
    () =>
      applySearch(
        items,
        q,
        q ? filters : { ...filters, year: "", genre: null, minScore: 0 },
      ),
    [items, q, filters],
  );
  ...
  async function loadMore() {
    ...
    try {
      const data = q
        ? await tmdb.searchCatalog(q, page + 1)
        : await catalog.browseFiltered(filters, page + 1);
      if (requestKey !== currentKey.current) return;
      setItems((list) => {
        const seen = new Set(
          list.map((item) => `${item.media_type}:${item.id}`),
        );
        return list.concat(
          data.results.filter(
      ...
      if (requestKey === currentKey.current) setError((err as Error).message);
    } finally {
      if (requestKey === currentKey.current) setLoading(false);
    }
  }
  ...
  return (
    <div className="rise">
      ...
      <h1 className="mt-1 text-3xl tracking-tight">
        {q || "Найти фильм, сериал или человека"}
      </h1>
      ...
      <div className="mt-5 space-y-3">
        <div className="flex flex-wrap gap-2">
          {(
            [
              ...
              ["movie", "Фильмы"],
              ["tv", "Сериалы"],
            ] as Array<[SearchKind, string]>
          ).map(([id, label]) => (
            <button
              key={id}
              className={chip(filters.kind === id)}
              onClick={() => setFilters((f) => ({ ...f, kind: id }))}
            >
              {label}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-2">
          {SCORES.map((n) => (
            <button
              key={n}
              className={chip(filters.minScore === n)}
      ...
      {loading && !items.length ? (
        ...
        <div className="mt-6 space-y-8">
                ...
                {people.map((person) => (
                  ...
                  >
                    {person.profile_path ? (
                      ...
                      <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-card text-sm text-mute">
          ...
          {shown.length ? (
            ...
            <div>
              <Empty text="Ничего не подошло на загруженных страницах. Сбрось фильтры или загрузи ещё." />
              {page < pages ? (
                <button
                  disabled={loading}
                  onClick={loadMore}
